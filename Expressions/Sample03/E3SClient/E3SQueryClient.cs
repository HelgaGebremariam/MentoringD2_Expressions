...
namespace Sample03.E3SClient
{
	public class E3SQueryClient
	{
		...
		public IEnumerable<T> SearchFTS<T>(int start = 0, int limit = 0, params string[] queries) where T : E3SEntity
		{
			HttpClient client = CreateClient();
			var requestGenerator = new FTSRequestGenerator(BaseAddress);

			Uri request = requestGenerator.GenerateRequestUrl<T>(queries.ToList(), start, limit);
		...
		public IEnumerable SearchFTS(Type type, int start = 0, int limit = 0, params string[] queries)
		{
			HttpClient client = CreateClient();
			var requestGenerator = new FTSRequestGenerator(BaseAddress);

			Uri request = requestGenerator.GenerateRequestUrl(type, queries.ToList(), start, limit);
